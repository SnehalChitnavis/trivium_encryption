...
int main()
{
  unsigned int key[80], iv[80], states[288], ct[60], Z_k[60], r_pt[60], rec_plain_text[NUM_BITS_PT], plain_text[NUM_BITS_PT], ciphertext[NUM_BITS_PT], z[NUM_BITS_PT], t1, t2, t3, temp, tempx;
  ...
  for (i = 0; i < 10; i++)
    printf("%0x",iv_h[i]);
    printf("\nPlaintext:\t");
  for (i = 0; i < NUM_BITS_PT/8; i++)
    printf("%0x",pt[i]);

  // Convert to byte arrays to bit stream
  for (i=0; i<10; i++){
	  for(j=0; j<8; j++){
  ...
  for(j=0; j<NUM_BITS_PT; j++){
	  ...
	  // Key generated for 1 bit
	  z[j] = t1 ^ t2 ^ t3;

	  // 1 bit of plain text encrypted
	  ciphertext[j] = plain_text[j] ^ z[j];
  ...
  // **************************************************
  // Decrypting the cipher text received with the same key generated. 
  // Since at the receiver end, same key and IV will generate same Z.
